...
local function adjust_opacity(windown, delta, warp)
	...
	opacity = warp and (opacity - math.floor(opacity)) or math.min(math.max(opacity, 0), 1)
...
local key_tables = {
	...
	show_launcher = {
		...
		{ key = "w", mods = "NONE", action = act.ShowLauncherArgs({ flags = "FUZZY|WORKSPACES" }) },
		{ key = "t", mods = "NONE", action = act.ShowLauncherArgs({ flags = "FUZZY|TABS" }) },
...
local keys = {
	...
	{ key = "s", mods = "CTRL|ALT", action = act.PaneSelect({ mode = "SwapWithActiveKeepFocus" }) },
